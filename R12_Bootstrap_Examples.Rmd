...
## Sampling Distribution of S^2 does not converge to any particular shape

If the population is normally distributed, then the sample variance will follow a Chi Square shape distribution
```{r}
#true variance = 25
samp.var <- replicate(10000, var(rnorm(25, 20.5, sd=5)))
hist(samp.var, probability=TRUE, breaks=50)

#In theory the shape of this distribution is a chi square with 24 degrees of freedom, rescaled
x<-0:60
lines(x=x, y=dchisq(x*24/25,24)*24/25)
```

This is not true when you have samples from some other population shape.

```{r}
#true variance = 25
n <- 10
df <- n-1
samp.var <- replicate(10000, var(rexp(n, 1)))
hist(samp.var, probability=TRUE, breaks=50)

#In theory the shape of this distribution is a chi square with 24 degrees of freedom, rescaled
x<-seq(0,6,by=.01)
lines(x=x, y=dchisq(x*df/1,df)*df/1)
```
...
* The classical statistical approach is to use a chi squared distribution for the sampling distribution of $S^2$ (this assumes the population is Normal)
