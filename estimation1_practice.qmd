...
a. Construct simultaneous method of moment estimators $\hat{a}$ and $\hat{b}$ from the facts that $E(X)=\frac{a+b}{2}$ and $Var(X)=\frac{(b-a)^2}{12}$.

::: {.callout-note collapse="true" title="Solution"}
As above, we can say that $\bar{X} \approx \frac{a+b}{2}$ and $S^2 \approx \frac{(b-a)^2}{12}$

We can rearrange these and state
$$2\bar{X} \approx a+b \quad\quad 12S^2 \approx b^2-2ab+a^2$$
A substitution of $b\approx 2\bar{X}-a$ can be made in the second equation. Using the quadratic formula (details are omitted) you get $\hat a = \bar{X} \pm \sqrt{3}S$. The same solution is found for $\hat b$ and it stands to reason then that the higher value is the estimator for $b$ while the lower value is the estimator for $a$.
...
mean(range.hat)-1 #bias

```
Repeating this a few times I find the estimated bias is between -0.015 and +.015 so I would venture to guess that the estimator is in deed unbiased.
:::
