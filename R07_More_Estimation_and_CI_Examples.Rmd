...
## Example: Estimating the maximum of a population
...
for(j in 1:NMC1){
  ...
  Mbelow[j] <- quantile(sim_S, .025) > .95
}
mean(Mbelow)
mean(containsM)
1-mean(Mbelow)-mean(containsM)
```
...
Turns out (from a little research https://en.wikipedia.org/wiki/German_tank_problem) that the lower bound for a 95% confidence interval would be the sample max $\max(X)$ and the upper bound would be $\max(X)/\alpha^{1/n}$ (for a 95% CI, $\alpha = .05$)
...
How well does the central limit theorem estimate the mean of a Uniform(0,1)
First, let's see how "normal" the sampling distribution of Xbar is.
...
my_means <- replicate(10000, generate_data(20))
...
my_means <- replicate(10000, generate_data(360, x, px))
hist(my_means, breaks=50)
```
...
### Assessing coverage rate
Let's see how well the coverage rate of the CLT interval is for the uniform

```{r}
NMC2 <- 10000 #how many times I replicate my CI calculation to 
             # assess coverage rate / accuracy

constructCI <- function(uniformData){
  #This constructs a 95% CI only, because 1.96 is hard-coded
  xbar <- mean(uniformData)
  s <- sd(uniformData)
  n <- length(uniformData)
  LB <- xbar - 1.96*s/sqrt(n)
  UB <- xbar + 1.96*s/sqrt(n)
  return(c(LB, UB))  
}

covered <- rep(FALSE, NMC2) #keep the results from my CI estimates
                            #here
for(i in 1:NMC2){
  uniformData <- runif(80, 0, 1)
  #note that pop mean = .5
  CI <- constructCI(uniformData)
  covered[i] <- CI[1] <= .5 & CI[2] >=.5
}
mean(covered) #this will say what proportion are TRUE in that vector
```
