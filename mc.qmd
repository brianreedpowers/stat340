...
[Stanisław Ulam](https://en.wikipedia.org/wiki/Stanislaw_Ulam) was a famous nuclear physicist (fun fact: after fleeing Europe during WWII, Ulam was a professor at UW-Madison before he was recruited to work on the Manhattan project at Los Alamos).
...
### Example: integrate $(x+1)^{-3}$ from 0 to $\infty$
...
```{r,include=F}
# fix seed not regenerating somehow
set.seed(5)
```
...
## MC estimator example: compute $\pi$
...
We *do* know that $\pi$ is the ratio of the area of a circle to radius².
...
![Estimating Pi](./images/PiBoard.png)
...
Well, the circle has area $\pi$ and the square has area $4$, so the probability is $p_{\text{in}} = \pi/4$.
...
<summary>**Bonus:** (click to show) alternate setup: find $E[1_\bigcirc(X,Y)]$ for indep. uniform $X,Y$</summary>
...
![Random Sampling and the Uniform Distribution](./images/sampling_uniform.gif)
